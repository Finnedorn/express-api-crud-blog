const validatePostData = (req, res, next) => {
    ...
    if (!image) {
        ...
        return res.status(400).json({ error: "Il campo 'image' deve essere una stringa o contenere un file" });
    }

    if (!tags) {
        return res.status(400).json({ error: "Il campo 'tags' è richiesto!" });
    } else if(!Array.isArray(tags) && tags.length > 0) {
        return res.status(400).json({ error: "Il campo 'tag' deve essere un Array con almeno 1 elemento" });
    }
